...
// Port of: src/decoders/html.rs
// Upstream commit: 499ae0f2ff649af84c921af4b008f7c617b0bf87
// Source SHA-256: 077427c2a4fe583f1e3241ffba89e9052e5c8f977ccc168e5f120b786ae6f134
...
public static class HtmlUtils
{
    ...
    // Rust: add_html_token
    public static void add_html_token(StringBuilder result, ReadOnlySpan<byte> token, bool add_space)
    {
        if (add_space)
        {
            result.Append(' ');
        ...
        if (token.Length >= 3 && token[0] == (byte)'&' && token[^1] == (byte)';')
        {
            var entity = token.Slice(1, token.Length - 2);
            uint? entity_code = null;

            if (entity.Length > 1 && entity[0] == (byte)'#')
            {
                var code = entity.Slice(1);
                int fromBase = 10;
                if (code.Length > 1 && (code[0] == (byte)'x' || code[0] == (byte)'X'))
                {
                    code = code.Slice(1);
                    fromBase = 16;
                }
                try
                {
                    string codeStr = System.Text.Encoding.ASCII.GetString(code);
                    entity_code = Convert.ToUInt32(codeStr, fromBase);
                }
                catch
                {
                    entity_code = null;
            ...
            {
                string entityStr = System.Text.Encoding.ASCII.GetString(entity);
                entity_code = lookup_entity(entityStr);
            }

            if (entity_code.HasValue)
            {
                try
                {
                    string charStr = char.ConvertFromUtf32((int)entity_code.Value);
            ...
            {
                result.Append(System.Text.Encoding.UTF8.GetString(token));
            }
        }
        else
        {
    ...
    // Rust: html_to_text
        // Rust: html_to_text
    public static string html_to_text(string input)
    {
        byte[] inputBytes = System.Text.Encoding.UTF8.GetBytes(input);
        var result = new StringBuilder(inputBytes.Length);
        ...
        {
            byte ch = inputBytes[pos];
            if (!in_comment)
            {
                switch (ch)
                {
                    case (byte)'<':
                        if (!in_tag && !in_head && !in_style && !in_script && !in_template && !is_token_start)
                        {
                            add_html_token(result, inputBytes.AsSpan(token_start, token_end - token_start + 1), is_after_space);
                    ...
                    case (byte)'/' when in_tag:
                        ...
                        continue;
                    case (byte)'!' when in_tag && tag_token_pos == 0:
                        if (pos + 2 < inputBytes.Length && inputBytes[pos + 1] == (byte)'-' && inputBytes[pos + 2] == (byte)'-')
                        {
                            in_comment = true;
                            continue;
                        }
                    ...
                    case (byte)' ' or (byte)'\t' or (byte)'\r' or (byte)'\n':
                        if (!in_tag && !in_head && !in_style && !in_script && !in_template)
                        {
                            if (!is_token_start)
                            {
                                add_html_token(result, inputBytes.AsSpan(token_start, token_end - token_start + 1), is_after_space && !is_new_line);
                                is_new_line = false;
                            }
                            is_after_space = true;
                        }
                        is_token_start = true;
                        continue;
                    case (byte)'&' when !in_tag && !is_token_start && !in_head:
                    ...
                    case (byte)';' when !in_tag && !is_token_start && !in_head:
                        ...
                        add_html_token(result, inputBytes.AsSpan(token_start, pos - token_start + 1), is_after_space && !is_new_line);
                        is_token_start = true;
                        is_after_space = false;
                        is_new_line = false;
                        continue;
    ...
    // Rust: text_to_html
    public static string text_to_html(string input)
    {
        byte[] inputBytes = System.Text.Encoding.UTF8.GetBytes(input);
        var result = new List<byte>(inputBytes.Length + 26);
        result.AddRange(System.Text.Encoding.ASCII.GetBytes("<html><body>"));
        foreach (byte ch in inputBytes)
        {
            switch (ch)
            {
                case (byte)'\n':
                    result.AddRange(System.Text.Encoding.ASCII.GetBytes("<br/>"));
                    break;
                case (byte)'<':
        ...
        result.AddRange(System.Text.Encoding.ASCII.GetBytes("</body></html>"));
...
{
    [TestMethod]
    public void convert_text_to_html()
    {
        ...
        {
            Assert.AreEqual(expected, HtmlUtils.text_to_html(input));
        }
    }

    [TestMethod]
    public void convert_html_to_text()
    {
        var inputs = new (string input, string expected)[]
        {
        ...
        foreach (var (input, expected) in inputs)
        {
            Assert.AreEqual(expected, HtmlUtils.html_to_text(input), $"Failed for '{input}'");
        }
    }

    [TestMethod]
    public void convert_html_entity()
    {
        var inputs = new (string input, string expected)[]
        {
            ("&lt;", "<"),
        ...
        foreach (var (input, expected) in inputs)
        {
            ...
            HtmlUtils.add_html_token(sb, System.Text.Encoding.UTF8.GetBytes(input), false);
            Assert.AreEqual(expected, sb.ToString(), $"Failed for '{input}'");
        }
    }

    [TestMethod]
    public void html_to_text_removes_style_content()
    {
        string input = "<style>body{color:red}</style><div>Hello</div>";
    ...
    [TestMethod]
    public void html_to_text_removes_script_content()
    {
        string input = "<style>body{color:red}</style><div>Hello</div><script>alert('x')</script><div>World</div>";
        string output = HtmlUtils.html_to_text(input);
        Assert.IsFalse(output.Contains("body{color:red}"));
        Assert.IsFalse(output.Contains("alert('x')"));
        Assert.IsTrue(output.Contains("Hello"));
        Assert.IsTrue(output.Contains("World"));
    }

    [TestMethod]
    public void html_to_text_removes_template_content()
    {
        string input = "<div>Hello</div><template><div>Hidden</div><style>.x{}</style></template><div>World</div>";
        string output = HtmlUtils.html_to_text(input);
        Assert.IsFalse(output.Contains("Hidden"));
        Assert.IsFalse(output.Contains(".x{}"));
        Assert.IsTrue(output.Contains("Hello"));
        Assert.IsTrue(output.Contains("World"));
    }
}
